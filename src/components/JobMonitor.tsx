...
export function JobMonitor() {
  ...
  const { data } = useQuery<ApiResponse<Job[]>>({
    queryKey: ['activity'], // Reusing activity key but filtering for jobs effectively
    queryFn: () => fetch('/api/activity').then(res => res.json()),
    refetchInterval: 3000
  });
  const jobs = (data?.data ?? []).slice(0, 5);
  const activeJobsCount = jobs.filter(j => j.status === 'info').length;
  return (
          ...
          >
            ...
            <div className="max-h-[300px] overflow-y-auto divide-y divide-white/5">
              {jobs.length === 0 ? (
                ...
                jobs.map((job: any) => (
                  <div key={job.id} className="p-4 hover:bg-white/5 transition-colors group">
      ...
      >
        <div className="flex items-center justify-between w-full px-2">
          <div className="flex items-center gap-3">
            <div className={`h-2 w-2 rounded-full ${activeJobsCount > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-zinc-400'}`} />
            <span className="text-sm font-bold tracking-tight">
              {activeJobsCount > 0 ? `${activeJobsCount} Jobs Running` : 'System Ready'}
            </span>
          </div>
